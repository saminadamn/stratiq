...
const ACTION_PLANS: Record<ActionPlanCategory, ActionPlanItem[]> = {
  REVENUE_DECLINE_ORDERS: [
    ...
    { day: 60, action: 'Test a limited-time acquisition offer on the highest-converting channel.' },
    {
      day: 90,
      action:
        'Reallocate marketing spend toward the channels that drove the most orders in the test.',
    },
  ],
  REVENUE_DECLINE_AOV: [
    {
      day: 30,
      action: 'Introduce bundle or cross-sell offers at checkout for top-selling products.',
    },
    {
      day: 60,
      action: 'Test a minimum-order free-shipping threshold above the current average order value.',
    },
    {
      day: 90,
      action: 'Roll out the highest-performing upsell offer across all customer segments.',
    },
  ],
  REVENUE_DECLINE_GENERIC: [
    {
      day: 30,
      action:
        'Review the revenue trend by category and region to isolate where the decline is concentrated.',
    },
  ...
  NEGATIVE_PROFIT_MARGIN: [
    ...
    { day: 60, action: 'Adjust pricing or discontinue the least profitable SKUs.' },
    {
      day: 90,
      action:
        'Renegotiate supplier terms or source lower-cost alternatives for the remaining low-margin lines.',
    },
  ],
  SLOW_INVENTORY_TURNOVER: [
  ...
  HIGH_CHURN_RISK: [
    {
      day: 30,
      action:
        'Reach out directly to the highest-risk customers with a personalized retention offer.',
    },
    {
      day: 60,
      action: 'Launch an automated re-engagement campaign for the broader at-risk segment.',
    },
    {
      day: 90,
      action: 'Review the onboarding/engagement flow for the behaviors most correlated with churn.',
    },
  ],
};
